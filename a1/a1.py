...
def shortest_path(g, src, max_depth):
    lvl = 0
    queue = {src: 1}
    paths = {src: [src]}
    node2num_paths = {src: 0}
    node2distances = {}
    node2distances[src] = 0
    node2parents = {}
    for v in g:
        node2parents[v] = []
        node2num_paths[v] = 0
        ...
        for vertex in current:
            for node in g[vertex]:
                if node not in paths:
                    num_paths = 1
                    paths[node] = paths[vertex] + [node]
                    node2distances[node] = lvl + 1
                    node2num_paths[vertex] = num_paths
                    node2parents[node].append(vertex)
                    queue[node] = 1

                if node2distances[node] == node2distances[vertex] + 1:
                    node2num_paths[node] = node2num_paths[vertex] + node2num_paths[node]
                    node2parents[node].append(vertex)
    ...
    parents = {}
    for x in g:
        parents[x] = list(set(node2parents[x]))
        if parents[x] == []:
            del parents[x]

    for y in g:
        if node2num_paths[y] == 0:
            del node2num_paths[y]

    return node2distances, node2num_paths, parents
...
def bfs(graph, root, max_depth):
    ...
    """
    ###TODO
    s,c,m=shortest_path(graph,root,max_depth)
    return s,c,m
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    d = dict(sorted(node2distances.items(),key=lambda x:x[0]))
    dis = sorted(d.items(),key=lambda x:x[1],reverse=True)
    list =[]
    el = {}
    x =[]
    for k2,vl in node2parents.items():
      x.append(k2)
    ...
    d =dict.fromkeys(node2distances,0.0)

    for j in range(len(dis)):
      u = dis[j][0]
      try:
        for p in node2parents[u]:
          sow = ((1.0+d[u])/node2num_paths[u]) * node2num_paths[p]
          if (p,u) not in el:
            el[(u,p)] +=sow
          else:
            el[(p,u)] +=sow
    ...
    Dt = dict(sorted(el.items(), key=lambda x:x[0][0]))
    return Dt
...
def approximate_betweenness(graph, max_depth):
    ...
    AB = {}
    AB=dict.fromkeys(graph.edges(),0.0)
    for node in graph.nodes():
      node2distance,node2num_paths,node2parents = bfs(graph,node,max_depth)
      AB_UP = bottom_up(node,node2distance,node2num_paths,node2parents)
      for F in AB_UP.keys() :
        if F in AB:
          AB[F] += AB_UP[F]
        else:
          AB[F] = AB_UP[F]
    for k2,val in AB.items():
      nval = AB[k2]/2.0
      AB[k2] = nval
    return AB
...
def partition_girvan_newman(graph, max_depth):
    ...
    betweenval = approximate_betweenness(graph,max_depth)
    components1 = []
    g1 = graph.copy()
    high = sorted(betweenval.items(),key=lambda x:x[1],reverse=True)
    components=[c for c in nx.connected_component_subgraphs(g1)]
    while len(components)== 1:
      g1.remove_edge(*high[0][0])
      del high[0]
      components=[c for c in nx.connected_component_subgraphs(g1)]
    for c in components:
      components1.append(c)
    return components1
...
def get_subgraph(graph, min_degree):
    ...
    subgraphnodes = [name for name, degree in degrees.items() if degree >= min_degree]
...
def norm_cut(S, T, graph):
    ...
    volume1 = float(volume(S, graph))
    volume2 = float(volume(T, graph))

    norm_cut1 = (float(cut_value / volume1) + float(cut_value / volume2))
...
def score_max_depths(graph, max_depths):
    ...
    list = []
    for k in (max_depths):
        d = partition_girvan_newman(graph, k)
        n_val = norm_cut(d[0], d[1], graph)
        list.append((k, n_val))
    return list
...
def make_training_graph(graph, test_node, n):
    ...
    g1 = graph.copy()
    s=g1.neighbors(test_node)
    nds = sorted(s)
    for m in range(n):
        g1.remove_edge(nds[m], test_node)
    return g1
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        if n != node and not graph.has_edge(node, n):
            neighbors3 = set(graph.neighbors(n))
            scores.append(((node, n), 1. * len(neighbors & neighbors3) / len(neighbors | neighbors3)))
    k3.sort(key=lambda a: a[0])
    k3.sort(key=lambda a: a[1], reverse=True)
    for i in range(k):
        kn.append(k3[i])
    return kn
...
def path_score(graph, root, k, beta):
    ...
    sow = []
    n = set(graph.nodes())
    node2distances, node2num_paths, node2parents = shortest_path(graph, root, k + 2)
    neighbors = set(graph.neighbors(root))
    for n1 in n.difference(neighbors):
        if n1 != root:
            sow.append(((root, n1), (beta ** node2distances[n1]) * node2num_paths[n1]))
    return sorted(sow, key=lambda x: x[1], reverse=True)[:k]
...
def evaluate(predicted_edges, graph):
    ...
    pass
    cnt = 0
    nPredictedEdges = len(predicted_edges)
    for eEdge in predicted_edges:
        if eEdge[0] in graph.neighbors(eEdge[1]):
            cnt += 1

    return (float(cnt) / nPredictedEdges)
