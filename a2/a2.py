...
def tokenize(doc, keep_internal_punct=False):
    ...
    if keep_internal_punct == False:
        k1 = re.sub('\W+', ' ', doc.lower()).split()
        sow1 = np.array(k1)
        return sow1
    else:
        k = re.sub("[^\w']", ' ', doc.lower()).split()
        sow = np.array(k)
        return sow
...
def token_pair_features(tokens, feats, k=3):
    ...
    a = []
    b = []
    for x in range(len(c) - k + 1):
        a.append(([c[x], c[x + 1], c[x + 2]]))
    for i in range(len(a)):
        for combo in combinations(a[i], 2):
            b.append('__'.join(combo))
    for sow in b:
        feats['token_pair=' + sow] += 1
...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for j in tokens_list:
        so = [g for h in range(len(set(j)))]
        r.append(so)
        g += 1
        for i, t in sorted(featurize(j, feature_fns)):
            vocab[i] += 1
            data.append(t)
    row = np.array(r)
    data1 = np.array(data)
    for k, val in sorted(vocab.items()):
        x.append((k, kc))
        kc += 1
    for ky, v in x:
        c.append((v))
    col = np.array(c)
    X = csr_matrix((data1, (row, col)), shape=(len(tokens_list), len(vocab)))  # .toarray()
    return X, vocab
...
def top_coefs(clf, label, n, vocab):
    ...
    while clfLen >= n:
        if i >= clfLen:
            ...
            if clf.coef_[0][i] >= 0:
                featurePos = vocab.get_feature_names()[i]
                coefficientPos = clf.coef_[0][i]
                positiveFile.append((featurePos, coefficientPos))
                counter2 = counter2 + 1

            else:
                featureNeg = vocab.get_feature_names()[i]
                coefficientNeg = clf.coef_[0][i]
                negativeFile.append((featureNeg, coefficientNeg))
                counter1 = counter1 + 1

        i = i + 1
    positiveFile = sorted(positiveFile, key=lambda: abs(coefficientPos), reverse=True)
    negativeFile = sorted(negativeFile, key=lambda: abs(coefficientNeg), reverse=True)

    if label == 1:
        return positiveFile[:n]
    elif label==0:
        return negativeFile[:n]
    else:
        pass
...
def print_top_misclassified(test_docs, test_labels, X_test, clf, n):
    ...
    for f in range(len(test_docs)):
        if predict[f] != test_labels[f]:
            createDic = {'truth': test_labels[f], 'predicted': predict[f], 'proba': proba[f]}
            List.append(createDic)
    List = sorted(List, key=lambda createDic: createDic['proba'][0] if createDic['predicted'] == 0 else
    createDic['proba'][1], reverse=True)
    print(List[:n])
